    ...
    <div>
      <div className="slide" style={{backgroundImage: `url(${image.src})`}}>
        <div className="slide-text container mx-auto flex flex-col items-start justify-center h-full gap-5 text-violet-50">
          ...
          <Link to={`/products/${image.category}`} className="slide-btn h-12 w-72 border uppercase border-violet-50 hover:text-orange-50 hover:border-orange-500 font-medium mt-4 lg:mt-5"><span className="z-[1] absolute text-center top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-full duration-300">{image.btn}</span></Link>
        </div>
      </div>
    </div>
  )
